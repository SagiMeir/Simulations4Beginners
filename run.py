...
for i, seedNum in enumerate([937142, 937143, 937144, 937145, 937146]):
  simForMetaD = Simulation(dt = 0.833E-15, R= np.array([[1,0,0]]) * 1E-10, Nsteps= 2000000, mass = 6.633E-26, kind = ["Ar"], fac = 1E10, xyzname = "simForMetaD_lowDeep" + str(i) + ".xyz", outname = "simForMetaD_lowDeep" + str(i) + ".log", momentname="simForMetaD_p_lowDeep" + str(i) + ".log", forcenamme="simForMetaD_F_lowDeep" + str(i) + ".log", mtype="NVT", ftype="DoubleWell", gamma= 2.226e13, startingStep=120000, withMetaD=True, w=6E-20, sigma=0.5e-12, MetaDfreq=5000, isDeep=True, seed=seedNum, gaussiansname="gaussiansPos_3rd_" + str(i) + ".log")
  simForMetaD.run()

# MetaD 2 Well (DEEP) with Poisson distribution - low MetaD rate (repeat)
for i, seedNum in enumerate([937142, 937143, 937144, 937145, 937146]):
  simForMetaD = Simulation(dt = 0.833E-15, R= np.array([[1,0,0]]) * 1E-10, Nsteps= 2000000, mass = 6.633E-26, kind = ["Ar"], fac = 1E10, xyzname = "simForMetaD_lowWithDistDeep" + str(i) + ".xyz", outname = "simForMetaD_lowWithDistDeep" + str(i) + ".log", momentname="simForMetaD_p_lowWithDistDeep" + str(i) + ".log", forcenamme="simForMetaD_F_lowWithDistDeep" + str(i) + ".log", mtype="NVT", ftype="DoubleWell", gamma= 2.226e13, startingStep=120000, withMetaD=True, w=6E-20, sigma=0.5e-12, MetaDfreq=5000, withPoissonDist=True, isDeep=True, seed=seedNum, gaussiansname="gaussiansPos_4th_" + str(i) + ".log")
  simForMetaD.run()
